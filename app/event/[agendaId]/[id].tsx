...
import { iconInformation, iconPlace, iconAccessibility, iconStatus, iconWordKey, iconChoiceLocation, iconApi, iconCroix, iconFavorite, iconLink, iconArrowDown, iconArrowUp } from '@/utils/imagesUtils';
        ...
        <View>
          <Text style={styles.modalDescription}>
            {isDescriptionExpanded
              ...
              : (event.longDescription?.fr?.substring(0, 180) || 'Aucune description disponible') + '...'}
          </Text>
          <TouchableOpacity
...
{/* CONDITIONS */}
        ...
        <Text style={styles.eventText}>{event.conditions?.fr ?? 'Aucune condition pour cet évènement.'}</Text>
        <View>
          {event.registration?.some(item => item.type === 'phone' || item.type === 'email') && (
            <>
              ...
              {event.registration.map((item, index) => {
                switch (item.type) {
                  case 'phone':
                    return <Text key={index} style={styles.eventText}>📱 {item.value}</Text>;
                  case 'email':
                    return <Text key={index} style={styles.eventText}>📧	{item.value}</Text>;
                  default:
                    return null;
  ...
  {/* HORAIRE DUREE */}
        ...
        <Text style={styles.eventText}>Durée estimée : {getEventDuration(event.firstTiming?.begin, event.firstTiming?.end)}</Text>
{/* LIEU DETAILS */}
...
{/* LIEU */}
 ...
 {/* ACCESSIBILITÉ */}
        <View style={styles.textInformation}>
          ...
          />
          <Text style={styles.eventText}>
            {(event.accessibility?.ii || event.accessibility?.hi || event.accessibility?.vi || event.accessibility?.pi || event.accessibility?.mi)
 ...
 {/* STATUS DE LEVENEMENT */}
      {event.status !== 1 && (
        <View style={styles.textInformation}>
          ...
          />
          <Text style={styles.eventText}>
            {event.status ? statusLabels[event.status] || 'Statut inconnu' : 'Statut non spécifié pour cet évènement.'}
          </Text>
        </View>
      )}
{/* API */}
 ...
 {/* Boutons "Aimer" et "Repartager" */}
          <TouchableOpacity style={styles.actionButton} onPress={() => console.log('Repartager l\'événement')}>
            <Image source={iconLink} style={styles.iconActionButton} alt="Repartager" />
          </TouchableOpacity>
          <TouchableOpacity style={styles.actionButtonTwo} onPress={() => console.log('Aimer l\'événement')}>
